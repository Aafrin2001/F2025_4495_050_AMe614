...
export class ActivityService {
  ...
  }, userId?: string): Promise<{ success: boolean; data?: Activity; error?: string }> {
    try {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          activitiesThisWeek: activities.filter(a => new Date(a.created_at) >= oneWeekAgo).length,
          activitiesThisMonth: activities.filter(a => new Date(a.created_at) >= oneMonthAgo).length,
        }
      };
    } catch (error: any) {
