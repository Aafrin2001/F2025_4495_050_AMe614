...
import { IoLogOutOutline, IoMedicalOutline, IoHeartOutline, IoWalkOutline, IoStatsChartOutline, IoCalendarOutline, IoWarningOutline, IoTimeOutline, IoCheckmarkCircleOutline, IoRocketOutline, IoPulseOutline, IoRefreshOutline, IoSettingsOutline, IoChevronForwardOutline } from 'react-icons/io5';
...
const MainPage: React.FC<MainPageProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      loadTodaySchedule();
    }, 60000); // Refresh schedule every minute
  ...
  const loadMedicationStats = async () => {
    const result = await MedicationService.getMedicationStats();
  ...
  const loadTodaySchedule = async () => {
    const result = await MedicationService.getTodaySchedule();
    if (result.success && result.data) {
  ...
  const loadHealthSummary = async () => {
    if (user?.id) {
      const result = await healthMetricsService.getLatestMetrics(user.id);
      if (result.data) {
  ...
  const loadCriticalAlerts = async () => {
    const result = await AdminDashboardService.getEmergencyAlerts();
    if (result.success && result.data) {
    ...
    <div className="main-page">
      <div className="main-header">
        <div className="header-content">
          <div>
            <h1>{getGreeting()}, {capitalizeName(user.firstName)}!</h1>
            <p className="header-subtitle">Here's your health overview for today</p>
          </div>
      ...
      <div className="main-content">
        ...
        {criticalAlerts.length > 0 && (
          <div className="critical-alerts-banner" onClick={() => navigate('/admin-dashboard')}>
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card warning" onClick={() => navigate('/admin-dashboard')}>
            ...
            <div className="stat-info">
              <div className="stat-value">{medicationStats?.overdueMedications || 0}</div>
        ...
        <div className="section-card quick-actions-section">
          ...
          <div className="quick-actions-grid">
            ...
            <div className="quick-action" onClick={() => navigate('/health-monitoring')}>
              ...
              <div className="action-label">Health Monitoring</div>
            </div>
            <div className="quick-action" onClick={() => navigate('/activities')}>
              <IoWalkOutline className="action-icon" />
